...
def remove_rular(arg, base):
    ...
    rects = [r for r in rects if (
        imgArea*args.maxRularArea) > (r[2]*r[3]) > (imgArea*args.minRularArea)]
...
def BorderLine(MaxBoundary, lines, index, flag):
    ...
    if getLine >= 3 and LastLine:
        if flag == "top":
            lineDetectH.append((LastLine[0], max(
                LastLine[1], LastLine[3]), LastLine[2], max(LastLine[1], LastLine[3])))
        if flag == "left":
            lineDetectV.append((max(LastLine[0], LastLine[2]), LastLine[1], max(
                LastLine[0], LastLine[2]), LastLine[3]))
        if flag == "bottom":
            lineDetectH.append((LastLine[0], min(
                LastLine[1], LastLine[3]), LastLine[2], min(LastLine[1], LastLine[3])))
        if flag == "right":
            lineDetectV.append((min(LastLine[0], LastLine[2]), LastLine[1], min(
                LastLine[0], LastLine[2]), LastLine[3]))
...
def main():
    ...
    parser.add_argument('--maxRularArea', type=float,
                        help='Consider maximum rular area')
    parser.add_argument('--minRularArea', type=float,
                        help='Consider minimum rular area')
    parser.add_argument('--rularWidth', type=float, help='maximum rular width')
    parser.add_argument('--positionLeft', type=float,
                        help='rular position in left')
    parser.add_argument('--positionRight', type=float,
                        help='rular position in right')
    parser.add_argument('--positionBelow', type=float,
                        help='rular position in below')
    parser.add_argument('--rularRatioMax', type=float,
                        help='rular position in below')
    parser.add_argument('--rularRatioMin', type=float,
                        help='rular position in below')
    parser.add_argument('--colSeparator', type=float,
                        help='consider space between column. 25% of width')
    parser.add_argument('--minArea', type=float,
                        help='rular position in below')
